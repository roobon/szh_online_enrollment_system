...
<?php
  	 ...
  	 if (isset($_GET['id'])){			
		$mydb->setQuery("SELECT * FROM schoolyr where IDNO= ".$_GET['id']." and `AY`='{$_SESSION['AY']}' AND SEMESTER='{$_SESSION['SEMESTER']}'");
							...
							<table id="example" class="table table-striped table-bordered table-hover table-responsive" cellspacing="0">
							  ...
							  <tbody>
							  	...
							  	global $mydb;
							  		...
							  		$mydb->setQuery("SELECT g.`IDNO`, `STUDENTNAME`, `DESCRIPTION`,`DAY`, `C_TIME`, `ROOM`, c.`SECTION`,c.INST_NAME FROM  class c RIGHT JOIN grades g  ON g.SUBJ_ID=c.SUBJ_ID WHERE g.IDNO=".$_GET['id']."");
							  		loadresult();
